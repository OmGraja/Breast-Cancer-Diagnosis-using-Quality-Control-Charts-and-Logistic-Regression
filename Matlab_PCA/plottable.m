...
numrow = xrows;
numcol = numcolgroup *round(xcols/sumcolgroup);
...
for row=1:numrow,
   ...
   for col=1:numcol,
      ...
      %ht(row,col) = imtext( (col-0.5)/numcol, (1-(row-0.5)/numrow),  s,'center');
      hh = imtext( (col-0.5)/numcol, (1-(row-0.5)/numrow),  s,'center');
      set(hh,'FontWeight','bold','FontName','times','fontsize',11,'FontAngle','normal');
...
%  set(gcf,'Color','white'); %camzoom(1.1); 
%  set(gcf,'InvertHardCopy','off'); axis image;
%  set(gcf,'PaperPositionMode','auto');